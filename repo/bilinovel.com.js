...
export default class extends Extension {
  ...
  lastLineDecrypt(rows) {
    ...
    let i = 1;
    if ((line.length > 0 && this.dict2[line[0]]) || (line.length > 1 && this.dict2[line[1]])) {
      rows[rows.length - 1] = line.replace(/./g, (c) => {
        if (this.marks.has(c)) return c;
        if (i++ % 3 == 0) return "";
        return this.dict2[c] ?? c;
      });
  ...
  async watch(url) {
    ...
    if (total > 1) {
      const rows = (await this.querySelectorAll(res, "#acontent > p, img")).map(e => this.filter.call(this, e));
