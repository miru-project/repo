...
export default class extends Extension {
    ...
    async requestWSetting(url) {
        const domain = await this.getSetting("domain_otakudesu");
    ...
    async search(query, page) {
        ...
        const page_domain = await this.getFullDomain();
        ...
        for (const element of element_item_search) {
            ...
            const urls = await this.getAttributeText(element.content, "h2 > a", "href");
            const url = urls.replace(page_domain, "");
            list_search.push({ title, url, cover });
        }
    ...
    async latest(page) {
        ...
        for (const element of element_latest) {
            ...
            const cover = await this.getAttributeText(element.content, "div > div.thumb > a > div > img", "src");
            const elementUrl = await this.getAttributeText(element.content, "div > div.thumb > a", "href");
            const url = await elementUrl.toString().replace(page_domain, "");
            list_item.push({ title, cover, url });
        }
    ...
    async detail(url) {
        ...
        const title = await this.querySelector(
            fetch_detail_anime,
            "#venkonten > div.venser > div.jdlrx > h1"
        ).text;
        ...
        let array_desc = [];
        const element_desc = await this.querySelectorAll(
            fetch_detail_anime,
            "#venkonten > div.venser > div.fotoanime > div.infozin > div > p"
        );
        ...
        for (const element_header of element_episode_list_header) {
            ...
            for (const element_episode of element_episode_list) {
                ...
                const urls_episode = await this.getAttributeText(element_episode.content, "span > a", "href");
                const url_episode = await urls_episode.toString().replace(page_domain, "");
                episodes_anime.push({ name: title_episode, url: url_episode });
            }
        }

        let episodes = quality.map((item_quality) => {
            ...
            return { title: item_quality, urls: temp_episode };
        });
    ...
    async watch(url) {
        ...
        const [cleanUrl, qualityRaw] = url.split("#");
        const quality = qualityRaw || "720p";

        const fetch_watch = await this.requestWSetting(cleanUrl);
        const element_options = await this.querySelectorAll(
            fetch_watch,
            `#embed_holder > div.mirrorstream > ul.m${quality} > li`
        ) || [];
        ...
        for (const element of element_options) {
            ...
            let stream_url = await this.getAttributeText(element.content, "a", "data-content");
            stream_title = stream_title.replaceAll(" ", "");
            if (list_stream.includes(stream_title)) {
                streamable_data.push({ stream_title, stream_url });
            }
        }

        for (const source of list_stream) {
            const found = streamable_data.find((item) => item.stream_title === source);
            if (found) {
                streamable = found;
                break;
            }
        ...
        const element_embed = await this.getAttributeText(fetch_embed_link, "iframe", "src");
