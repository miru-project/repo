...
// @lang         zh-cn
// @license      MIT
// @icon         https://haiwaikan.com/tp2_statics/img/favicon.ico
// @package      vod.api.json.haiwaikan
// @type         bangumi
// @webSite      https://haiwaikan.com
// ==/MiruExtension==
...
class Ext extends VodExtension {
  ...
  async search(kw, page) {
    const res = await this.callApi({ query: kw, page })
    const ids = res.list.map(item => item.vod_id).join()
    return super.search(ids, page)
  }
}
