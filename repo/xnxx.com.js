...
export default class extends Extension {
  async latest(page) {
    ...
    for (const element of bsxList) {
      ...
      if (!title) {
        title = await this.getAttributeText(html, "img", "alt") || "XNXX Video";
      }
  ...
  async detail(url) {
    ...
    const title = titleEl ? (await titleEl.getAttributeText("content") || await titleEl.text).trim() : "XNXX Video";
    ...
    const descEl = await this.querySelector(res, "p.metadata-row.video-description, meta[property='og:description']");
