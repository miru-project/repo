...
export default class extends Extension {
    ...
    Decrypt(data, key, iv) {
      ...
      let result = this.CryptoJS.AES.decrypt(data, key, {
        iv: iv,
        mode: this.CryptoJS.mode.CBC,
      });
      let text = this.CryptoJS.enc.Utf8.stringify(result).toString();
      return text;
    }
    ...
    async latest(page) {
      let keyword = `/api/v3/comics?free_type=1&limit=24&offset=${
        page - 1
      }*20&_update=true&ordering=popular`;
    ...
    async watch(url) {
      ...
      let key = "xxxmanga.woo.key";
      let value = this.CryptoJS.enc.Hex.parse(result);
      let minyMin = this.CryptoJS.enc.Base64.stringify(value);
