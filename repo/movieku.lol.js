...
var CryptoJSAesJson = {
  ...
  async search(kw, page,filter) {
    ...
    bsxList.forEach((element) => {
      const url = element.match(/href="https:\/\/movieku.lol(.+?)"/)[1];
  ...
  async latest(page) {
    const res = await this.request(`/latest-movies/page/${page}/`);
    const bsxList = res.match(/<article[\s\S]+?<\/article>/g)
    const videos = [];
    bsxList.forEach((element) => {
      const url = element.match(/href="https:\/\/movieku.lol(.+?)"/)[1];
  ...
  async detail(url) {
    ...
    const cover = res.match(/(https:\/\/i\d.wp.com[^\?]+?)"/)[1];
    const desc = res.match(/<p>([\S\s]+?)<\/p>/)[1];
    const ep_list_area = res.match(/<ul style[\s\S]+?<\/ul>/);
    if (ep_list_area === null) {

      const player_link = res.match(/<iframe src="(.+?)"/);
      if (player_link === null) {
        return {
          title,
          cover,
  ...
  async watch(url) {
    ...
    // 
      const req_url = url.split(";")[0];
      const resolution = url.split(";")[1]; 

      const res = await this.request(req_url);
      const player_link = res.match(/<iframe src="(.+?)"/);
      const response = await this.get_content(player_link[1]);
