...
export default class extends Extension {
  async latest() {
    ...
    const list = res.match(/<div class="text-center text-sm relative drak:bg-black">([\s\S]+?)<\/span>/g);
    const bangumi = [];
    list.forEach((element) => {
      console.log(element);
      const url = element.match(/onclick="window.open\('(.+?)'/)[1];
      const title = element.match(/<span class="leading-loose line-clamp-1 mt-2 2k:text-xl">(.+?)<\/span>/)[1];
      const cover = element.match(/style="background-image: url\('(.+?)'\)"/)[1];
      bangumi.push({
        title,
        url,
        cover,
  ...
  async search(kw, page) {
    ...
    const list = res.match(/<div class="flex flex-row min-h-64 my-4 relative">([\s\S]+?)<\/b>/g);
    const data = [];
    list.forEach((e) => {
      const title = e.match(/<p class="text-2xl hidden xl:inline-block">([\s\S]+?)<\/p>/)[1];
      const cover = e.match(/style="background-image: url\('(.+?)'\)"/)[1];
  ...
  async detail(url) {
    ...
    const cover = res.match(/style="background-image: url\('(.+?)'\)"/)[1];
    const desc = res.match(/<div id="vod_content" class="hidden xl:inline-block">([\s\S]+?)<\/div>/)[1]?.replace("<!-- 影片简介 -->", "").trim();
