...
export default class extends Extension {
  async load() {
    ...
    this.decrypt = (src, key1, key2) => {
      let prefix = new Array(key2.length);
      for (let i = 0; i < key2.length; i++) {
        prefix[key1[i]] = key2[i];
      }
      let a = this.CryptoJS.MD5(prefix.join("") + "YLwJVbXw77pk2eOrAnFdBo2c3mWkLtodMni2wk81GCnP94ZltW").toString(),
        key = this.CryptoJS.enc.Utf8.parse(a.substring(16)),
        iv = this.CryptoJS.enc.Utf8.parse(a.substring(0, 16)),
        dec = this.CryptoJS.AES.decrypt(src, key, {
          iv: iv,
          mode: this.CryptoJS.mode.CBC,
          padding: this.CryptoJS.pad.Pkcs7,
        });
      return dec.toString(this.CryptoJS.enc.Utf8);
    };
    this.base64decode = (str) => {
      var words = this.CryptoJS.enc.Base64.parse(str);
